...
while true; do
    BLOCK_HEIGHT=$(docker exec -it competency_test_p2pool-bitcoin-1 bitcoin-cli -rpcconnect=127.0.0.1 -rpcport=18443 -rpcuser=alice -rpcpassword=password getblockheight 2>/dev/null || true)
    if [[ -n "$BLOCK_HEIGHT" ]]; then
        echo "RPC is available. Current block height: $BLOCK_HEIGHT"
        break
    else
